...
import chapter08_classes.constructor.Constructor;

public class Student {

    int studentCode;
    String name;
    double score;       //필드만 입력해도 기본생성자는  default로 생성된다


    // 기본 생성자의 정의 ->  생성자는 클래스명과 같다
    Student(){
    ...
    Student(int studentCode){
        ...
        this.studentCode = studentCode;// 얘 뭐지
    }
    Student(String name){
        System.out.println("String 매개변수 생성자로 객체가 생성되었습니다");
        this.name = name;
    }// this의 의미는 객체명.속성명 에서 객체명 부분을 의미함 -> 아직 객체 생성전이기 때문에
    //객체이름을 대입할 수 없어서 this라는 임의적인 대명사를 사용.
    ...
    Student(String name, int studentCode){
        System.out.println("int, String 매개변수 생성자로 객체가 생성되었습니다");
        this.studentCode = studentCode;
        this.name = name;
    }
    ...
    void showInFo(){
        ...
        //객체의 속성을 참조하는 것이기 때문에 가독성을 위해 this를 적는다
    }
